<template>
	<view id="carList">
		...
		<view class="choose" @touchmove="touchmove">
			...
			<view :class="[showChooseIndex == 1?'choose-show':'','priceChoose']">
					...
					<view class="uni-list">
						<block v-for="(list, key) in list" :key="list">
							<block v-if="list.data.length">
								...
								<view class="one" v-for="(carname,index) in list.data" :key="carname" :class="list.data.length -1 == index ? 'uni-list-cell-last' : ''"
				...
				<view class="uni-indexed-list-bar" :class="touchmove ? 'active' : ''" @touchstart="touchStart" @touchmove="touchMove"
				 @touchend="touchEnd" @touchcancel="touchCancel" :style="{height:winHeight/26 + 'px'}">
					<text v-for="(list,key) in list" :key="key" class="uni-indexed-list-text" :class="touchmoveIndex == key ? 'active' : ''"
			...
			<view :class="[showChooseIndex == 2?'choose-show':'','priceChoose']">
				<scroll-view class="scrollList" scroll-y :scroll-into-view="scrollViewId" :style="{height:winHeight/26 + 'px'}">
					<view class="one">
						...
						<view class="oneChild" v-for="(item,index) in cheXingList" :key="index" v-if="cheXingList.length != 0" @tap="getCheXi(item.sid,item.sname)">
							{{ item.sname }}
						</view>
					</view>
				</scroll-view>
			...
			<view :class="[showChooseIndex == 3?'choose-show':'','priceChoose']">
				<scroll-view class="scrollList" scroll-y :scroll-into-view="scrollViewId" :style="{height:winHeight/26 + 'px'}">
					...
					<view class="one">
						...
						<view class="oneChild" v-for="(item,index) in cheXiList" :key="index" v-if="cheXiList.length != 0" @tap="getCheXiLists(item.name)">
							{{ item.name }}
						</view>
					</view>
				</scroll-view>
		...
		<view class="cheLiangList">
			<view class="cheLiangList-Item" v-for="(item,index) in carList" :key="index" @tap="$goWindow('../carInfo/carBasicInfo/carBasicInfo?id='+item.id)">
				<view class="cheLiangList-Item-Left">
					<image :lazy-load="true" :src="item.coverUrl[0]" mode="aspectFill"></image>
				</view>
				<view class="cheLiangList-Item-Right">
					<view class="carTitle">{{ item.brandName }}</view>
		...
		<share  frind="../../../../static/share/frind.png" frind_circle="../../../../static/share/frind_circle.png"
		 @update_share_show="get_update_share_show" :if_show_share="if_show_share" :share_url="null" :car_id="null" :userId="userId"
		 car_title="信车智联" car_summary="快来看，好多好车啊！！！" img_url="http://wshop.xczlbox.com/static/logo.jpg"></share>
	</view>
</template>
...
<script>
	...
	export default {
		...
		onNavigationBarButtonTap() {
			...
			console.log(this.if_show_share)
		},
		onLoad() {
			this.isMarket = 0;
			this.getCarBrand();
		...
		methods: {
			...
			async getCarLists() {
				try {
					if (this.ifHaveCar == '已加载全部数据' && this.pageNum != 1) {
						return;
					} else {
						this.$loading();
						let orderStr = '';
...
<style>
	...
	.cheLiangList-Item-Right>view:nth-child(2) {
		width: 100%;
		overflow: hidden;
		white-space: nowrap;
