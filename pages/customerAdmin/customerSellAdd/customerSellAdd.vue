	...
	<view id="addSellCar">
		...
		<view class="content">
			...
			<view class="content_item">
				...
				<view class="content_item_right">
					<input type="number" placeholder="请输入客户电话" placeholder-class="pclass" v-model="cuPhone" />
				</view>
			</view>
			<view class="content_item" @tap="openPickerView(6)">
			...
			<view style="width: 100%; border-bottom: 1upx solid #E8E8E8;" v-for="(item,index) in carList" :key="index">
				...
				<view class="content_item" @tap="openPickerView(5,index)">
					<view class="content_item_left">车型</view>
					<view class="content_item_right">
			...
			<view class="content_item">
				<view class="content_item_left">里程</view>
				<view class="content_item_right">
					<input type="text" placeholder="请输入里程" placeholder-class="pclass" v-model="mileage" />
					<text>万公里</text>
				</view>
			</view>
			<view class="content_item" @tap="openPickerView(3)">
				<view class="content_item_left">外观颜色</view>
				<view class="content_item_right">
					<text>{{ color }}</text>
				</view>
			</view>
			<view class="content_item" @tap="openPickerView(4)">
				<view class="content_item_left">上牌时间</view>
				<view class="content_item_right">
		...
		<view :class="[pickerViewIndex == 4?'openPicker':'','picker-view']">
			<view class="chooseSure">
				<text @tap="openPickerView('==')">取消</text>
				<test>上牌时间</test>
				<text @tap="sureTime()">确认</text>
			</view>
			<picker-view @change="bindChangeTime">
				<picker-view-column>
		...
		<view :class="[pickerViewIndex == 5?'openPicker':'','picker-view']">
			<view class="chooseSure">
				<text @tap="openPickerView('==')">取消</text>
				<test>车型</test>
				<text @tap="sureCarType()">确认</text>
			</view>
			<picker-view @change="bindChangeCarType">
				<picker-view-column>
					<view class="item" v-for="(item,index) in carType" :key="index">{{item.name}}</view>
				</picker-view-column>
			</picker-view>
		</view>
		<!-- 客户来源选择器 -->
...
<script>
	...
	import { getBrand, getXinCheXings, getSelectByAudiId } from '@/api/share.js'
	import { customerCustomerSellAdd, customerCustomerSellModify } from '@/api/customer.js'
	export default {
		data() {
			const years = [];
			const months = [];
			...
			return {
				...
				carcars:[],
				carList:[
					{brand:'',brandName:'请选择品牌',cars:'',carsName:'请选择车系',model:'',modelName:'请选择车型',carBrandIndex:0,carcarsIndex:0,carModeIndex:0}
				],
				carListIndex:'==',
				carNameIndex:0,
		...
		onLoad(params) {
			...
			this.color = newParams.customerSell.color
			this.imgPath = newParams.customerSell.imgPath
			this.registrationTime = newParams.customerSell.registrationTime.substr(0,10)
		...
		methods: {
			...
			// 添加卖车客户
			async customerCustomerSellAdds(){
				try{
					if(this.cuName == ''){
						this.$toast('请输入客户名称')
					}else if(this.cuSex == ''){
						...
						if(this.carList.length != 0){
							for(let i in this.carList){
								let jsonOne = {
									...
									model:this.carList[i].model,
									modelName:this.carList[i].modelName
								}
								customerCars = jsonOne
							}
						}
						let params = {
							customerSell:{
							...
							customerCar:customerCars
						}
						let resData;
						let state = '添加'
						if(this.id != ''){
							params.customerSell.id = this.id
							state = '修改'
							resData = await customerCustomerSellModify(params)
						}else{
							resData = await customerCustomerSellAdd(params)
						}
						if(resData.code === 200){
							this.$toast(`${state}成功`)
							if(state == '修改'){
			...
			// 确定车型事件
			sureCarType(){
				this.carList[this.carListIndex].modelName = this.carType[this.carList[this.carListIndex].carModeIndex].name
				this.carList[this.carListIndex].model = this.carType[this.carList[this.carListIndex].carModeIndex].id
				this.openPickerView('==')
			},
			// 修改车型事件
			bindChangeCarType(e){
				this.carList[this.carListIndex].carModeIndex = e.detail.value[0]
			},
			// 获取车型
			async getSelectByAudiId(){
				try{
					let params = {audiId: this.carList[this.carListIndex].cars};
					let resData = await getSelectByAudiId(params)
					if(resData.code === 200){
						this.carType = resData.result
					}else{
						this.$toast(resData.message)
					}
				}catch(e){
			...
			sureTime(){
				...
				this.registrationTime = this.years[this.yearsIndex]+'-'+month
				this.openPickerView('==')
			},
			// 修改上牌时间事件
			bindChangeTime(e){
			...
			// 确定车系事件
			surecars(){
				this.carList[this.carListIndex].cars = this.carcars[this.carList[this.carListIndex].carcarsIndex].sid
				this.carList[this.carListIndex].carsName = this.carcars[this.carList[this.carListIndex].carcarsIndex].sname
				this.carList[this.carListIndex].modelName = '请选择车型'
				this.carList[this.carListIndex].model = ''
				this.getSelectByAudiId()
				this.openPickerView('==')
			},
			// 修改车系事件
			bindChangecars(e){
			...
			sureBrand(){
				if(oldBarand != this.carList[this.carListIndex].carBrandIndex || oldBarandIndex != this.carListIndex){
					...
					this.carList[this.carListIndex].cars = ''
					this.carList[this.carListIndex].modelName = '请选择车型'
					this.carList[this.carListIndex].model = ''
				}
				this.openPickerView('==')
			...
			openPickerView(index,carListIndex){
				if(index == 2 && this.carList[carListIndex].brandName == brand){
					this.$toast('请先选择品牌')
				}else if(index == 5 && this.carList[carListIndex].carsName == cars){
					this.$toast('请先选择车系')
				}else{
					if(index != '=='){
