...
<template>
	<view id="login">
		<view class="login-img">
			<image src="../../../static/index/login/loginLogo.png"></image>
		</view>
		<view :class="[indexState == 0?'input_choose':'','inp_box','border_bottom']" @tap="chooseIndex(0)">
			<image src="../../../static/index/login/userName.png" mode=""></image>
			<input type="text" placeholder="请输入用户名/手机号" :placeholder-style="placeholderStyle" v-model="username" />
		</view>
		<view :class="[indexState == 1?'input_choose':'','inp_box','border_bottom']" @tap="chooseIndex(1)">
			<image src="../../../static/index/login/passWord.png" mode=""></image>
			<input type="password" placeholder="请输入密码" :placeholder-style="placeholderStyle" v-model="password" />
		</view>
		<view class="inp_end">
			<button class="login-button" type="primary" @tap="loginApi()">登录</button>
		</view>
		<view class="end_phone" @tap="$goPhone('010-5226-9086')">
			<image src="../../../static/index/login/phone.png" mode=""></image>
			010-5226-9086
		</view>
	</view>
</template>
	...
	export default {
		data() {
			return {
				...
				password: this.$store.state.user.userPassword,
				loginStyle: "",
				indexState: '0',
		...
		methods: {
			...
			async loginApi() {
				if (this.username == '') {
					...
					uni.request({
						url: 'http://47.105.165.101:8686/xinche/app/login/AUTH_SHOPAPP_LOGIN',
						method: 'POST',
						dataType: 'json',
						data: {
			...
			async getUserInfoApi() {
				try {
					...
					if (resData.code === 200) {
						...
						if ((resData.result.merchantId == null || resData.result.merchantId == '') && (resData.result.marketId == null ||
								resData.result.marketId == '')) {
							uni.setStorageSync('qx', 'admine');
						} else if (resData.result.marketId != null && (resData.result.merchantId == null || resData.result.merchantId == '')) {
							uni.setStorageSync('qx', 'market');
						} else if (resData.result.merchantId != null && resData.result.merchantId != '') {
							uni.setStorageSync('qx', 'merchan');
							uni.setStorageSync('marketId', resData.result.marketId);
							uni.setStorageSync('merchantId',resData.result.merchantId)
						}
